...
When a candidate starts the test, Equip picks **one question at random** from the pool you selected. Different candidates therefore see different problems, so a question shared in one batch does not help the next candidate.
