...
These are the test types you can add from the **Tests** tab when creating an assessment, listed in the order they appear in the builder.
...
| **CSS Test** | Front-end styling: candidates write CSS to reproduce a target design, compared at desktop, tablet, and mobile breakpoints. | No |
...
Most test types are scored automatically the moment a candidate submits. A few need a human reviewer.
...
| **Not scored** | Psychometric |

**Psychometric tests** are subjective, so they carry no points and are excluded from the overall score. Results appear as a personality graph instead. The **Typing Test** also carries no weightage in the overall score.
