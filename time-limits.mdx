...
<Steps>
  <Step title="Open Assessment Settings">
    From the Recruiter Dashboard, open your assessment's settings and go to the **Settings** tab.
  </Step>
...
Candidates see a persistent countdown while taking a test, so remaining time is always visible. In one-by-one quizzes, candidates cannot return to earlier questions, so time spent on a question is final.
