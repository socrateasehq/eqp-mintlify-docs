...
|---|---|
| **Randomize the question order** | For quizzes that show questions all at once, each candidate sees the questions in a different order. Question-bank quizzes, such as quizzes built on Equip's content, are inherently randomized because the questions themselves are picked at random. Randomization is also available for [Programming and SQL tests](/randomize-coding-questions). |
...
| **Auto-submit on Tab-switch** | If the candidate switches to a different tab more than a number of times you set, the test is submitted automatically. |
