...
If you have a folder of CVs from a job board, an agency, or an old ATS, you do not need to add candidates one by one. **Bulk CV Import** lets you upload up to 100 CVs in a single batch. Equip parses each file and creates a job application automatically.

## Uploading CVs

Each batch is uploaded into a specific job opening, so every parsed CV becomes an application in that job's pipeline.

<Steps>
  <Step title="Open the job opening">
    Go to the job opening you want to add candidates to. All applications created by the import will appear in this job's candidate pipeline.
  </Step>
  <Step title="Select your PDF files">
    ...
    <!-- screenshot: bulk CV upload dialog showing "Select PDF files to upload" with the PDF-only and file limits hint -->
  </Step>
...
<Note>
Common failure reasons are a missing email, a missing name, a duplicate application for the same job, or a CV the parser could not read. Fix the file or the duplicate, then re-upload just the affected CVs in a new batch.
</Note>

## Unverified Candidates

Candidates added through Bulk CV Import never confirmed their email address themselves, so Equip marks them as **unverified**. On the candidate profile, an amber icon next to the email shows "Email not verified".
...
Each imported application also records its **source** as "Bulk CV Upload", so you can filter the pipeline by source and compare imported candidates against direct applicants in your analytics.
