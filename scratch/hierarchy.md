# ⚡ Equip in 10 Sentences (single top-level article, no category)

# ATS

## Job Openings

- Creating a Job Opening
  - The Job Post
  - The Application Form
  - The Candidate Pipeline
- Sharing Job Posts
- Your Careers Page
- What Job Boards Does Equip Publish To?

## Resume Screening

- What is the Job Fit Score?
- How CV Parsing Works
- Bulk CV Import

## Applications Dashboard

- Candidates and CVs
- Candidate Profile
- Changing Stages (includes section: The Email That Gets Sent)
- Pipeline Stages
- Scorecards
- Email Templates

## Insights & Rediscovery

- ATS Insights: Hiring Analytics
- Talent Rediscovery
- Candidate Search

# Assessments

## Create

- How to Create an Assessment
- Assessments, Tests, and Questions: The Hierarchy
- Equip Content vs Custom Content
- Testing for Multiple Skills at Once
- Determining Assessment Duration
- Time Limits on Tests
- Assessment Settings Explained

## Tests & Questions

- Test Types in an Assessment
- Question Difficulty Levels and Years of Experience
- Question Limits per Test Type
- Do Candidates See the Same or Random Questions?
- Screen-by-Screen vs All-at-Once Quizzes
- Can't Find a Role or Skill to Assess For?
- Equip Test Library
  - Supported Programming Languages
  - Psychometric Tests: DISC and Big Five
  - The Communication (CEFR) Test
- Custom Content
  - Creating Custom Tests
  - Question Types in Custom Quizzes
  - AI-Graded Questions: Text, Audio, and Video Answers
  - Multilingual Tests
  - Creating a Custom Question Bank Quiz
  - Import Questions Using Excel
  - Creating a Custom Programming Test
  - Randomizing Programming and SQL Questions
  - Setting Time Limits for a Custom Quiz
  - Using Documents in Custom Tests
  - Adding Video Response Questions
  - Issues with Custom Coding Challenges

## Run

- Previewing an Assessment
- Tracking Live Progress of Your Assessment
- Archiving an Assessment

## Evaluate

- Test Scores and Overall Score: How Assessments Are Evaluated
- How Tests Are Graded on Equip
- How to Manually Grade Tests
- Negative Marking: When It Applies
- What Is a Good Assessment Score?
- Understanding CEFR Adaptive Test Scores
- Why Psychometric Tests Don't Have a Score
- Downloading Results and Reports
- Write Results to Google Sheets
- Why Are Some Assessment Results Missing?
- Why Can't I See Quiz Questions?

# AI Interviews

## Interview Types

- One-Way vs Conversational AI Interviews
- Interview Languages
- The Workspace: Code and Written Answers

## Creating Interviews

- Creating a One-Way AI Interview
- Creating a Conversational AI Interview
- Questions, Topics, and Scenarios
- Interview Settings

## Reports & Templates

- AI Interview Reports
- Custom Report Templates

# Candidate Experience

## Invitations & Access

- Sharing via URL vs Email Invitations
- Sending Email Invitations
- Email Status: Delivered, Opened, Clicked
- Changing the Sender Name and Email
- How Invitation Links Work
- Invitations vs Previews
- Can a Candidate Re-Attempt?
- Redirect URL After Completion

## Taking Tests & Interviews

- Taking an Assessment on Equip
- Taking an AI Interview
- Candidate Login Methods
- Device and Browser Compatibility
- Language Support on Equip
- Demo and Practice Tests
- How to Resume Your Attempt

## Proctoring & Anti-Cheating

- Ensuring Candidates Don't Cheat: AI Proctoring
- Proctoring Settings and How to Change Them
- Anti-Cheating Settings
- Disabling Camera and Mic Tracking
- Candidate Session Recording
- ChatGPT and AI Usage Detection
- What Is the Trust Score?
- ID Verification and Impersonation Detection

## Troubleshooting

- A Candidate Faced (or Is Facing) Issues
- Candidate Says They Submitted, But I Can't See It

# Account & Billing

## Credits & Billing

- How Credits Work and How to Purchase Them
- Credit Deduction Policy
- Reclaiming Credits from Unattempted Invitations
- First Purchase Discount
- TDS for Purchases
- Viewing and Downloading Invoices
- Changing Business Information After an Invoice
- Demo and Trial Accounts

## Team & Organization

- Adding and Removing Team Members and Admins
- Organization Settings

## Integrations & API

- API Integrations
- Using Equip Assessments with Lever
- Webhooks

## Support & Policies

- FAQs
- Contacting Equip Support
- Custom Service Agreements (SLAs)
- Security, Privacy & Candidate Data
